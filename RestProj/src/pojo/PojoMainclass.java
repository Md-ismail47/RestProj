...
public class PojoMainclass
{
	public static void main(String[] args) throws InterruptedException 
	{
		...
		GetCourse coursedata = given().queryParam("access_token",accessToken).expect().defaultParser(Parser.JSON).
		when().get("https://rahulshettyacademy.com/getCourse.php").as(GetCourse.class);
		System.out.println(coursedata.getLinkedIn());
		int courseAutomationsize = coursedata.getCourses().getWebAutomation().size();
		// All courses under WebAutomation
		for(int i=0;i<courseAutomationsize;i++)
		{
			System.out.println(coursedata.getCourses().getWebAutomation().get(i).getPrice());
			System.out.println(coursedata.getCourses().getWebAutomation().get(i).getCourseTitle());
		}
		for(int i=0;i<courseAutomationsize;i++)
		{
			if(coursedata.getCourses().getWebAutomation().get(i).getCourseTitle().equals("Protractor"))
			{
				System.out.println(coursedata.getCourses().getWebAutomation().get(i).getPrice());
			}
		}
	//	System.out.println(coursedata.getCourses().getWebAutomation().get(0).getCourseTitle());
	}
}
